        ...
        <header>
            <nav className="flex flex-col sm:flex-row items-center justify-between text-lg px-6 sm:px-12 mt-2 py-4">
                ...
                <div className="flex items-center text-gray-400 font-medium space-x-4 mt-3 sm:mt-0">
                    <Link className={`rounded-sm p-1 hover:underline ${theme==='dark' && 'text-white'}`} to="/">Home</Link>
                    <Link className={`rounded-sm p-1 hover:underline ${theme==='dark'&& 'text-white'}`} to="/Freelances">Profils</Link>
                    <Link className="text-white p-2 bg-indigo-600 rounded-3xl" to="/survey/1">faire le test</Link>
                    <button
                        onClick={toggleTheme}
                        className="ml-2 rounded-full p-1 border text-sm cursor-pointer hover:bg-gray-200 hover:border-gray-400 transition-colors"
